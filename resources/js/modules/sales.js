...
export class SalesModule {
    ...
    renderTable(sales) {
        ...
        tbody.innerHTML = sales.map(s => {
            ...
            return '<tr><td><button class="btn btn-link btn-sm p-0" data-action="view" data-id="' + s.id + '">' + this.esc(s.reference_number ?? '#' + s.id) + '</button></td>' +
                '<td>' + this.esc(cust) + '</td>' +
    ...
    showModal(sale = null) {
        ...
        vb.classList.add('d-none'); vb.innerHTML = '';
    ...
    async viewSale(id) {
        try {
            ...
            const rows = (s.items ?? []).map(i =>
                '<tr><td>' + this.esc(i.product?.name ?? '—') + '</td><td class="text-center">' + i.quantity + '</td>' +
                '<td class="text-end">' + parseFloat(i.unit_price ?? 0).toFixed(2) + '</td>' +
                '<td class="text-end">' + parseFloat(i.subtotal ?? (i.quantity * (i.unit_price ?? 0))).toFixed(2) + '</td></tr>'
            ).join('');
            vb.innerHTML = '<div class="row g-3 mb-3">' +
                ...
                '<div class="col-md-9"><strong>Notes:</strong><br>' + this.esc(s.notes ?? '—') + '</div></div>' +
                '<table class="table table-sm"><thead class="table-light"><tr><th>Product</th><th class="text-center">Qty</th><th class="text-end">Unit Price</th><th class="text-end">Subtotal</th></tr></thead>' +
                '<tbody>' + rows + '</tbody>' +
                '<tfoot><tr><td colspan="3" class="text-end fw-bold">Total:</td><td class="text-end fw-bold">' + parseFloat(s.total_amount ?? 0).toFixed(2) + '</td></tr></tfoot></table>';
            document.getElementById('saleModalLabel').textContent = 'Sale ' + (s.reference_number ?? '#' + s.id);
            new bootstrap.Modal(document.getElementById('saleModal')).show();
        } catch { window.utils?.showToast('Failed to load sale.', 'error'); }
    }

    async editSale(id) {
        try { const res = await apiClient.get('/sales/' + id); this.showModal(res.data?.data); }
        catch { window.utils?.showToast('Failed to load sale.', 'error'); }
    }
    ...
    addItemRow(item = null) {
        ...
        const opts = this.products.map(p =>
            '<option value="' + p.id + '" data-price="' + (p.selling_price ?? p.unit_price ?? 0) + '"' + (item?.product_id == p.id ? ' selected' : '') + '>' + this.esc(p.name) + '</option>'
        ).join('');
        tr.innerHTML = '<td><select class="form-select form-select-sm item-product"><option value="">Select product...</option>' + opts + '</select></td>' +
    ...
    async save() {
        ...
        try {
            if (this.editingId) { await apiClient.put('/sales/' + this.editingId, payload); window.utils?.showToast('Sale updated.', 'success'); }
            else { await apiClient.post('/sales', payload); window.utils?.showToast('Sale created.', 'success'); }
            bootstrap.Modal.getInstance(document.getElementById('saleModal'))?.hide();
            this.loadSales();
        } catch (err) {
            if (err.response?.status === 422) window.utils?.displayValidationErrors(err.response.data.errors ?? {}, form);
    ...
    async deleteSale(id, ref) {
        ...
        if (role === 'manager') {
            ...
            const ok = await window.utils?.showConfirmModal('Delete Sale', 'Delete sale <strong>' + this.esc(ref) + '</strong>?', 'Delete', 'btn-danger');
            if (ok) await this.doDelete(id);
        }
    }

    async doDelete(id) {
        try { await apiClient.delete('/sales/' + id); window.utils?.showToast('Sale deleted.', 'success'); this.loadSales(); }
        catch (err) { if (err.response?.status !== 401 && err.response?.status !== 403) window.utils?.showToast(err.response?.data?.message ?? 'Failed to delete.', 'error'); }
    }
